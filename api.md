...
This page lists all API calls implemented in the FeedHQ API. It is important to read and understand the [terminology](https://feedhq.readthedocs.io/en/latest/api/terminology.html) before refering to this API reference.
...
- `c`: the _continuation_ string (see below).
...
_Continuation_ is used for pagination. When FeedHQ returns a page, it contains a `continuation` key that can be passed as a `c` parameter to fetch the next page.
...
- (optional) `xt`, `it`, `nt` and `ot` are supported like in the API call.
...
- (optional) `ts`: an epoch timestamp **in microseconds**. When provided, only items _older_ than this timestamp are marked as read.
